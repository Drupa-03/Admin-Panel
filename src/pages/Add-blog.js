...
//       },
//     ]);
//     // Scroll to the last section after adding
//     setTimeout(() => {
...
export default function AddBlogPage() {
    ...
    meta_image: Yup.mixed().when("meta_image_preview", {
      ...
      then: (schema) =>
        schema
          ...
          .test("fileType", "Only JPG, PNG, WEBP, or AVIF files are allowed", (value) =>
        ...
        const parsedSections = blog.trnData.map((section, index) => ({
          ...
          content: Array.isArray(section.content_description)
            ? section.content_description.map((c, idx) => ({
                title: c.sub_title || "",
                description: c.description || "",
                isFirstItem: idx === 0, // First content item in each section is required
              }))
            : [{ title: "", description: "", isFirstItem: true }], // Fallback if content_description is not an array
          showPoints: section.content_points?.length > 0,
  ...
  const handleSubmit = useCallback(
    async (values) => {
      try {
        ...
        formData.append("blog_title", values.blog_title);
        formData.append("status", "1");
        if (values.meta_image) formData.append("title_image", values.meta_image);

        const trnData = values.contentSections.map((section, index) => {
          if (section.image) formData.append(`content_img[${index}]`, section.image);
  ...
  const removeSection = useCallback(
    (sectionIndex) => {
      ...
      formik.setFieldValue(
        "contentSections",
        formik.values.contentSections.filter((_, index) => index !== sectionIndex)
      );
    },
    [formik]
  );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 lg:pl-64 pt-12 px-4 sm:px-6 md:px-8">
      <div className="max-w-6xl mx-auto py-10">
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        value={item.title}
                        onChange={(e) =>
                          handleContentChange(e, sectionIndex, contentIndex, "title")
                        }
                        onBlur={() =>
                          formik.setFieldTouched(
                            `contentSections[${sectionIndex}].content[${contentIndex}].title`,
                    ...
                    <div className="space-y-1">
                      ...
                      <textarea
                        ...
                        value={item.description}
                        onChange={(e) =>
                          handleContentChange(e, sectionIndex, contentIndex, "description")
                        }
                        onBlur={() =>
                          formik.setFieldTouched(
                            `contentSections[${sectionIndex}].content[${contentIndex}].description`,
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <input
                            ...
                            value={item.title}
                            onChange={(e) =>
                              handlePointChange(e, sectionIndex, pointIndex, "title")
                            }
                            onBlur={() =>
                              formik.setFieldTouched(
                                `contentSections[${sectionIndex}].points[${pointIndex}].title`,
                        ...
                        <textarea
                          ...
                          value={item.description}
                          onChange={(e) =>
                            handlePointChange(e, sectionIndex, pointIndex, "description")
                          }
                          rows={3}
                          className="w-full p-3 border-2 border-gray-300 rounded-md"
                        />
          ...
          {formik.errors.contentSections && typeof formik.errors.contentSections === "string" && (
            <p className="text-red-500 text-sm mt-1">
              {formik.errors.contentSections}
            </p>
          )}
