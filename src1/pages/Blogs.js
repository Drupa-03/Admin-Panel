...
export default function UploadedBlogs() {
  const { is_view, is_add, is_update, is_delete } = usePermission("manage_blogs");
  ...
  const handleBulkDelete = async () => {
    if (confirm(`Are you sure you want to delete ${selectedBlogs.size} blog(s)?`)) {
      try {
        const deletePromises = Array.from(selectedBlogs).map((id) =>
          api.delete(`/nodesetup/meta_blog/${id}`)
        );
        await Promise.all(deletePromises);
        setBlogs((prevBlogs) => prevBlogs.filter((blog) => !selectedBlogs.has(blog.id)));
  ...
  const filteredBlogs = blogs.filter((blog) => {
    ...
    const titleMatch = normalize(blog.title).includes(search);
    const slugMatch = normalize(blog.slug_title).includes(search);
    const categoryMatch =
        ...
        <div className='bg-white dark:bg-gray-800 border border-[#004b8f]/20 rounded-2xl shadow p-4 mb-6'>
          <div className='flex flex-col sm:flex-row justify-between items-center gap-4'>
            <div className='relative w-full sm:w-auto'>
              ...
              <input
                type='text'
                placeholder='Search slug title, blogs...'
                value={searchTerm}
...
// import usePermission from "./hooks/usePermission";
// import ErrorPage from "./_error1";
// import { updateAccessToken } from "../utills/api"; // or "../utils/auth"
...
// //   const slugMatch = blog.slug_title?.toLowerCase().includes(search);
// //   const categoryMatch =
// //     filterCategory === "All" || blog.category === filterCategory;
