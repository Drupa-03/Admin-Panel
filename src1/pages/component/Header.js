...
import { getUserRole, logout, getAccessToken } from "@/utills/auth";
...
export default function Header() {
  ...
  useEffect(() => {
    const fetchUserInitial = async () => {
      const { id, user_type } = getUserRole();
      const token = getAccessToken();
      if (!id || !token || isAccessTokenExpired(token)) {
        console.warn("Invalid token or user ID, logging out:", { id, token });
        logout();
        return;
      }
