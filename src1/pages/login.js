...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.status === 200 && data.error_type === "success") {
        const { token, refresh_token, user_type, role_id, user_id, id } = data.data;

        if (!token || !refresh_token) {
          throw new Error("Missing tokens in response");
        }

        const userId = user_id || id; // Handle both user_id and id
        if (!userId) {
      ...
      const errorMessage = err.response?.data?.data || "Invalid mobile number or password";
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setLoading(false);
